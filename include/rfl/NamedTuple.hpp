...
/// A named tuple behaves like tuplet::tuple,
/// but the fields have explicit names, which
/// allows for reflection.
template <class... FieldTypes>
class NamedTuple {
 public:
  using Fields = tuplet::tuple<std::decay_t<FieldTypes>...>;
  using Values = tuplet::tuple<typename std::decay<FieldTypes>::type::Type...>;
 ...
 public:
  ...
  NamedTuple(FieldTypes&&... _fields)
      : values_({std::move(_fields.value_)...}) {
    static_assert(no_duplicate_field_names(),
                  "Duplicate field names are not allowed");
  }

  /// Construct from the fields.
  NamedTuple(const FieldTypes&... _fields) : values_({_fields.value_...}) {
    static_assert(no_duplicate_field_names(),
                  "Duplicate field names are not allowed");
  }
  ...
  NamedTuple(std::tuple<FieldTypes...>&& _tup)
      : NamedTuple(std::make_from_tuple<NamedTuple<FieldTypes...>>(
            std::forward<tuplet::tuple<FieldTypes...>>(_tup))) {
    static_assert(no_duplicate_field_names(),
                  "Duplicate field names are not allowed");
  }
  ...
  /// Construct from a tuple containing fields.
  NamedTuple(Fields&& _tup)
      : values_(_tup.map([](auto&& _f) { return std::move(_f.value_); })) {
    static_assert(no_duplicate_field_names(),
                  "Duplicate field names are not allowed");
  }

  /// Construct from a tuple containing fields.
  NamedTuple(const Fields& _tup)
      : values_(_tup.map([]<class F>(const F& _f) { return _f.value_; })) {
    static_assert(no_duplicate_field_names(),
                  "Duplicate field names are not allowed");
  }
  ...
  /// Template specialization for tuplet::tuple, so we can pass fields from
  /// other named tuples.
  template <class... TupContent, class... Tail>
  auto add(tuplet::tuple<TupContent...>&& _tuple, Tail&&... _tail) {
    if constexpr (sizeof...(Tail) > 0) {
      return add_tuple(std::forward<tuplet::tuple<TupContent...>>(_tuple))
          .add(std::forward<Tail>(_tail)...);
    } else {
      return add_tuple(std::forward<tuplet::tuple<TupContent...>>(_tuple));
    }
  ...
  /// other named tuples.
  template <class... TupContent, class... Tail>
  auto add(tuplet::tuple<TupContent...>&& _tuple, Tail&&... _tail) const {
    if constexpr (sizeof...(Tail) > 0) {
      return add_tuple(std::forward<tuplet::tuple<TupContent...>>(_tuple))
          .add(std::forward<Tail>(_tail)...);
    } else {
  ...
  auto add(NamedTuple<TupContent...>&& _named_tuple, Tail&&... _tail) {
    return add(
        std::forward<tuplet::tuple<TupContent...>>(_named_tuple.fields()),
  ...
  /// other named tuples.
  template <class... TupContent, class... Tail>
  auto replace(tuplet::tuple<TupContent...>&& _tuple, Tail&&... _tail) {
    if constexpr (sizeof...(Tail) > 0) {
      return replace_tuple(std::forward<tuplet::tuple<TupContent...>>(_tuple))
  ...
  /// other named tuples.
  template <class... TupContent, class... Tail>
  auto replace(tuplet::tuple<TupContent...>&& _tuple, Tail&&... _tail) const {
    if constexpr (sizeof...(Tail) > 0) {
      return replace_tuple(std::forward<tuplet::tuple<TupContent...>>(_tuple))
          .replace(std::forward<Tail>(_tail)...);
    } else {
  ...
  /// Returns the underlying tuplet::tuple.
 ...
 private:
  ...
  constexpr auto add_tuple(tuplet::tuple<TupContent...>&& _tuple) {
    const auto a = [this](auto&&... _fields) {
      return this->add(std::forward<TupContent>(_fields)...);
    };
    return tuplet::apply(a, std::forward<tuplet::tuple<TupContent...>>(_tuple));
  }

  /// Adds the elements of a tuple to a newly created named tuple,
  /// and other elements to a newly created named tuple.
  template <class... TupContent>
  constexpr auto add_tuple(tuplet::tuple<TupContent...>&& _tuple) const {
    const auto a = [this](auto&&... _fields) {
      return this->add(std::forward<TupContent>(_fields)...);
    };
    return tuplet::apply(a, std::forward<tuplet::tuple<TupContent...>>(_tuple));
  ...
  auto make_fields(Args&&... _args) {
    ...
    if constexpr (retrieved_all_fields) {
      return tuplet::tuple<std::decay_t<Args>...>{std::forward<Args>(_args)...};
    } else {
      ...
      return make_fields<num_additional_fields>(
          FieldType(std::forward<T>(tuplet::get<i>(values_))),
  ...
  auto make_fields(Args&&... _args) const {
    ...
    if constexpr (retrieved_all_fields) {
      ...
      using FieldType = typename std::tuple_element<i, Fields>::type;
      return make_fields<num_additional_fields>(
          FieldType(tuplet::get<i>(values_)), std::forward<Args>(_args)...);
    }
  }
    ...
    if constexpr (retrieved_all_fields) {
      ...
      if constexpr (size == _index) {
        ...
        return make_replaced<_index, V, T>(
            std::forward<V>(_values), std::forward<T>(_val),
            std::forward<Args>(_args)...,
            FieldType(std::forward<U>(tuplet::get<size>(_values))));
      }
    }
  }
  ...
  auto replace_tuple(tuplet::tuple<TupContent...>&& _tuple) {
    const auto r = [this](auto&&... _fields) {
      return this->replace(std::forward<TupContent>(_fields)...);
    };
    return tuplet::apply(r, std::forward<tuplet::tuple<TupContent...>>(_tuple));
  }

  /// Adds the elements of a tuple to a newly created named tuple,
  /// and other elements to a newly created named tuple.
  template <class... TupContent, size_t... Is>
  auto replace_tuple(tuplet::tuple<TupContent...>&& _tuple) const {
    const auto r = [this](auto&&... _fields) {
      return this->replace(std::forward<TupContent>(_fields)...);
    };
    return tuplet::apply(r, std::forward<tuplet::tuple<TupContent...>>(_tuple));
  ...
  constexpr static Fields retrieve_fields(
      tuplet::tuple<OtherFieldTypes...>&& _other_fields, Args&&... _args) {
    constexpr auto size = sizeof...(Args);

    constexpr bool retrieved_all_fields = size == std::tuple_size<Fields>{};

    if constexpr (retrieved_all_fields) {
      return Fields{std::forward<Args>(_args)...};
    } else {
      constexpr auto field_name = std::tuple_element<size, Fields>::type::name_;

      constexpr auto index =
          internal::find_index<field_name, tuplet::tuple<OtherFieldTypes...>>();
      ...
      return retrieve_fields(
          std::forward<tuplet::tuple<OtherFieldTypes...>>(_other_fields),
          std::forward<Args>(_args)...,
          FieldType(std::forward<T>(tuplet::get<index>(_other_fields).value_)));
    }
  }
 ...
 private:
  ...
  /// runtime overhead over a normal tuplet::tuple.
  Values values_;
};
